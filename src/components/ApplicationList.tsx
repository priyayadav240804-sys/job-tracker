...
export type SortOption = 'date-desc' | 'date-asc' | 'overdue' | 'company-asc';

const SORT_STORAGE_KEY = 'job_tracker_sort';
...
export default function ApplicationList({
  ...
  useEffect(() => {
    try {
      ...
      if (savedSort && ['date-desc', 'date-asc', 'overdue', 'company-asc'].includes(savedSort)) {
        setSortBy(savedSort);
      }
    } catch (e) {
      console.error('Failed to load sort setting from localStorage:', e);
    }
  }, []);
  ...
  const handleSortChange = (newSort: SortOption) => {
    ...
    try {
      ...
      console.error('Failed to save sort setting to localStorage:', e);
    }
  };
  ...
  const sortedApplications = [...filteredApplications].sort((a, b) => {
    if (sortBy === 'date-desc') {
      return b.dateApplied.localeCompare(a.dateApplied) || (b.createdAt - a.createdAt);
    }
    if (sortBy === 'date-asc') {
      return a.dateApplied.localeCompare(b.dateApplied) || (a.createdAt - b.createdAt);
    }
    if (sortBy === 'company-asc') {
      return a.company.localeCompare(b.company, undefined, { sensitivity: 'base' });
    }
    if (sortBy === 'overdue') {
      const aNeeds = isFollowUpNeeded(a, settings.followUpDaysThreshold);
      const bNeeds = isFollowUpNeeded(b, settings.followUpDaysThreshold);
      ...
      // Non-overdue applications follow, sorted by date applied newest first
      return b.dateApplied.localeCompare(a.dateApplied) || (b.createdAt - a.createdAt);
    }
    return 0;
  });
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-slate-200 dark:border-slate-800">
        <div className="flex items-center gap-2.5">
          ...
          <span className="text-xs px-2.5 py-0.5 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 font-semibold border border-slate-200 dark:border-slate-700">
            {isFiltering ? `${sortedApplications.length} of ${applications.length}` : applications.length}
          </span>
        </div>
      ...
      {/* Search Input, Sort Control, and Status Filter Controls */}
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5">
            ...
            <div className="flex items-center gap-2 self-end sm:self-auto shrink-0 w-full sm:w-auto">
              <label htmlFor="sort-select" className="text-xs font-semibold text-slate-500 dark:text-slate-400 whitespace-nowrap flex items-center gap-1">
                <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <select
                ...
                className="flex-1 sm:flex-none text-xs px-3 py-2.5 rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-900 dark:text-white font-medium focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500 focus:outline-none cursor-pointer shadow-xs"
                aria-label="Sort applications"
              >
                <option value="date-desc">Date applied (newest first)</option>
                <option value="date-asc">Date applied (oldest first)</option>
                <option value="overdue">Most overdue for follow-up</option>
                <option value="company-asc">Company name (A-Z)</option>
              </select>
            </div>
          </div>

          {/* Row 2: Status Filter Buttons / Control */}
      ...
      {applications.length === 0 ? (
        ...
        <div className="overflow-x-auto pb-4 -mx-4 sm:mx-0 px-4 sm:px-0">
          <div className="flex md:grid md:grid-cols-5 gap-3.5 min-w-[780px] md:min-w-0 items-start">
            {ALL_STATUSES.map((st) => {
              const columnApps = sortedApplications.filter((a) => a.status === st);
